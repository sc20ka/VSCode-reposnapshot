...
class ProductManager {
    ...
    addProduct(name, price, desc, img, category) {
        ...
        if (window) {
            try {
                window.ga("send", "event", "Products", "Create", name);
            }
            catch (e) {
    ...
    updateProduct(id, data) {
        ...
        if (product.status === "discontinued") {
            this.sendEmailToSubscribers(product.id, "Product Discontinued", `The product ${product.name} has been discontinued.`);
        }
        return product;
    }
    deleteProduct(id) {
        ...
        if (window && window.cache) {
            window.cache.invalidate("products");
        }
        return true;
    }
    // Email functionality mixed in the product manager
    sendEmailToSubscribers(productId, subject, body) {
    ...
    searchProducts(query, filters = {}) {
        ...
        DATABASE.products.forEach((product) => {
            let matches = product.name.toLowerCase().includes(query.toLowerCase()) ||
...
//# sourceMappingURL=next-edit-9-5.js.map