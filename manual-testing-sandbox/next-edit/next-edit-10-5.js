...
const util = {
    generateId: (prefix) => `${prefix}_${Math.random().toString(36).substring(2, 15)}${Date.now()}`,
    calculateProgress: (tasks) => {
        const completed = tasks.filter((t) => t.status === "completed").length;
        return tasks.length ? Math.round((completed / tasks.length) * 100) : 0;
    },
    sendNotification: (type, message, user) => {
...
function validateTask(task) {
    ...
    const userTasks = globalTasks.filter((t) => t.assignedTo === task.assignedTo && t.status !== "completed");
    if (userTasks.length >= _config.maxTasksPerUser) {
        throw new Error(`User cannot have more than ${_config.maxTasksPerUser} active tasks`);
    }
    return true;
}
...
function loadUserData(userId) {
    ...
    globalTasks = globalTasks.filter((t) => t.createdBy === userId || t.assignedTo === userId);
    globalProjects = globalProjects.filter((p) => p.members.includes(userId) || p.createdBy === userId);
...
class TaskManager {
    ...
    createTask(taskData) {
        ...
        if (task.assignedTo !== currentUser.id) {
            ...
            if (assignee) {
                console.log(`Sending email to ${assignee.email} about new task assignment`);
                util.sendNotification("task_assigned", `You've been assigned a new task: ${task.title}`, assignee);
            }
        }
    ...
    updateTask(taskId, updates) {
            ...
            if (updates.status === "completed") {
                ...
                if (user) {
                    ...
                    if (user.completedTasks >= 10 &&
                        ...
                        util.sendNotification("achievement", "You earned the Productive badge!", user);
                    }
                }
            }
        }
        ...
        if (updates.assignedTo && updates.assignedTo !== task.assignedTo) {
            ...
            if (assignee) {
                util.sendNotification("task_assigned", `You've been assigned to task: ${task.title}`, assignee);
            }
        }
    ...
    deleteTask(taskId) {
        ...
        if (task.projectId) {
            ...
            if (project) {
                ...
                setTimeout(() => {
                    const projectTasks = globalTasks.filter((t) => t.projectId === project.id && t.id !== taskId);
                    project.progress = util.calculateProgress(projectTasks);
                }, 0);
            }
        }
        ...
        if (task.assignedTo !== currentUser.id) {
            const assignee = globalUsers.find((u) => u.id === task.assignedTo);
            if (assignee) {
                util.sendNotification("task_deleted", `A task assigned to you was deleted: ${task.title}`, assignee);
            }
        }
    ...
    searchTasks(criteria) {
        ...
        let results = globalTasks.filter((task) => {
            ...
            const isVisible = task.createdBy === currentUser.id ||
                task.assignedTo === currentUser.id ||
                (task.projectId &&
                    globalProjects.some((p) => p.id === task.projectId &&
        ...
        if (criteria.sortBy) {
            results.sort((a, b) => {
                if (criteria.sortBy === "dueDate") {
                    if (!a.dueDate)
                        return criteria.sortDir === "asc" ? 1 : -1;
                    if (!b.dueDate)
                        return criteria.sortDir === "asc" ? -1 : 1;
...
//# sourceMappingURL=next-edit-10-5.js.map